...
serve(async (req) => {
  ...
  try {
    ...
    if (hasPrevRef || hasOrigRef) {
      // All scenes: use Gemini image model with reference(s)
      ...
      if (hasOrigRef) {
        ...
        const refLabel = idx === 0
          ? "[ORIGINAL REFERENCE — This shows the room/building in its current or completed state. Use it ONLY to understand the room layout, dimensions, window positions, door positions, wall locations, camera angle, and architectural identity. Generate an ABANDONED version of this exact same room — same proportions, same camera angle, same layout — but in a state of extreme decay and neglect.]"
          : "[ORIGINAL REFERENCE — This is the ORIGINAL STARTING STATE of the room/building before any renovation. Use it ONLY to preserve: room identity, room dimensions, wall positions, window count, window positions, door positions, floor opening positions, camera angle, and architectural character. Do NOT use this image to make the current scene look more finished or polished. The renovation must progress step by step, not jump to a completed state.]";
        contentParts.push({ type: "text", text: refLabel });
        contentParts.push({
          type: "image_url",
      ...
      // Add previous scene image (scenes 2-8)
      if (hasPrevRef) {
